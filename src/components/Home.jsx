...
export const Home = () => {
  ...
  const [newDate, setNewDate] = useState(new Date().getHours() + ":" + new Date().getMinutes() + ":" + new Date().getSeconds());
  
  const dbValue = collection(db, 'Users');

  const handleCreate = async (e) => {
    e.preventDefault();
    await addDoc(dbValue, { name: name, surname: surname, time: newDate });
